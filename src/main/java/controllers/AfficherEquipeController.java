...
public class AfficherEquipeController implements Initializable {
    ...
    private void callExistingUser(ActionEvent event) {
        ...
        try {
            Utilisateur utilisateur = serviceUtilisateur.getUserByEmail(selectedEmail);
            if (utilisateur != null) {
                // Populate text fields with user information
                tfmembre1.setText(utilisateur.getEmail());
...
// Populate storedEmails with data from your database or another source
    ...
    private void fetchStoredEmails() {
        List<Utilisateur> utilisateurs = new ArrayList<>(); // Create a new list to hold users
        // Manually add users to the list
        utilisateurs.add(new Utilisateur(1, "John", "Doe", "john@example.com", "password", "image1.jpg", "niveau1", "client", "123 Street", "Male"));
        utilisateurs.add(new Utilisateur(2, "Jane", "Doe", "jane@example.com", "password", "image2.jpg", "niveau2", "client", "456 Avenue", "Female"));
        utilisateurs.add(new Utilisateur(3, "Alice", "Smith", "alice@example.com", "password", "image3.jpg", "niveau3", "client", "789 Road", "Female"));
            ...
            if (utilisateur != null) {
                ...
                for (TextField textField : memberTextFields) {
                    if (textField.getText().isEmpty()) {
                        textField.setText(utilisateur.getEmail());
                        textField.setUserData(utilisateur); // Optionally store user data for later use
                        break;
                    }
                }
            } else {
