...
public class ServiceEquipeX implements IServiceX<Equipe> {
    ...
    public void ajouterMembre(Equipe equipe, Utilisateurss utilisateurss) throws SQLException {
        String req = "INSERT INTO equipe_members (equipe_id, user_id) VALUES (?, ?)";
        try (PreparedStatement pre = con.prepareStatement(req)) {
            pre.setInt(1, equipe.getID());
            pre.setInt(2, utilisateurss.getId());
            pre.executeUpdate();
        }
    }
    public void supprimerMembre(Equipe equipe, Utilisateurss utilisateurss) throws SQLException {
    ...
    public List<Utilisateurss> getEquipeMembres(Equipe equipe) throws SQLException {
        List<Utilisateurss> membres = new ArrayList<>();
